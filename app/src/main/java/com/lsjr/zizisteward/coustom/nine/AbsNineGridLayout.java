...
public abstract class AbsNineGridLayout extends ViewGroup {
    ...
    public AbsNineGridLayout(Context context) {
        super(context);
        init(context);
    }

    public AbsNineGridLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.AbsNineGridLayout);

        mSpacing = typedArray.getDimension(R.styleable.AbsNineGridLayout_sapcing, DEFUALT_SPACING);
        typedArray.recycle();
        init(context);
    }
    ...
    private void init(Context context) {
        mContext = context;
        if (getListSize(mUrlList) == 0 && getBitmaps(bitmaps) == 0) {
            setVisibility(GONE);
        }
    ...
    private void refreshByBitmap() {
        L_.e("直接从缓存设置图片 refreshByBitmap");
        removeAllViews();
        int size = getBitmaps(bitmaps);
        if (size > 0) {
            setVisibility(VISIBLE);
        } else {
            setVisibility(GONE);
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!mIsShowAll) {
                if (i < MAX_COUNT - 1) {
                    ...
                    layoutImageViewtoBitmap(imageView, i, bitmap, false);
                } else { //第9张时
                    if (size <= MAX_COUNT) {//刚好第9张
                        ...
                        imageView = createImageView(i);
                        layoutImageViewtoBitmap(imageView, i, bitmap, true);
                        break;
                    }
                }
            } else {
    ...
    private RatioImageView createImageView(final int i) {
        RatioImageView imageView = new RatioImageView(mContext);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        return imageView;
    }
     ...
     */
    private void layoutImageViewtoBitmap(RatioImageView imageView, int i, Bitmap bitmap, boolean showNumFlag) {
...
//        params.width = width;
    ...
    private int getBitmaps(List<Bitmap> bitmaps) {
     ...
     * @return true 代表按照九宫格默认大小显示，false 代表按照自定义宽高显示
     */
   // protected abstract boolean displayOneImage(RatioImageView imageView, String url, int parentWidth,NineGridLayout.DisplayImageSucceedListener displayImageSucceedListeners);

   // protected abstract void displayImage(RatioImageView imageView, String url,NineGridLayout.DisplayImageSucceedListener displayImageSucceedListeners);
