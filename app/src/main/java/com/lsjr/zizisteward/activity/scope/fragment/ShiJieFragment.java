...
import static com.lzy.okgo.utils.OkLogger.e;

/**
 * Created by admin on 2017/5/8.
 */

public class ShiJieFragment extends BaseMvpFragment<ShijiePresenter> implements IShijieView {
    ...
    protected void initView() {
        ...
        shiJieAdapter = new ShiJieAdapter(getContext().getApplicationContext(), mData);
        //shiJieAdapter.setHasStableIds(true);
        recyclerView.setAdapter(shiJieAdapter);
        shiJieAdapter.setHeaderView(getHeaderView(), recyclerView);
        shiJieAdapter.setCustomLoadMoreView(new XRefreshViewFooter(getContext()));
        ...
        xRefreshView.setOnRecyclerViewScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    if (isBottom = shiJieAdapter.getItemCount() - 1 == lastVisibleItem) {
                        pullStatus = ON_LOAD;
                        pageNum++;
        ...
        shiJieAdapter.setOnUserOperationListener(new ShiJieAdapter.onUserOperationListener() {
            @Override
            public void onLike(View view, int position) {
                currentPostion = position;
                if (mData.get(position).isIscollect()) {
                    /*取消收藏*/
                    onMackUnCollectforNet(position);
                } else {
                    onMackCollectforNet(position);
                }
            }

            @Override
            public void onCollect(View view, int position) {
                currentPostion = position;
                if (mData.get(position).isIscollect()) {
                    /*取消收藏*/
                    onMackUnCollectforNet(position);
    ...
    public void onMackCollectforNet(int position) {
        HashMap<String, String> map = new HashMap<>();
        map.put("OPT", "68");
        map.put("user_id", Config.USER_ID);
        map.put("share_id", mData.get(position).getId());
        createPresenter().loadMackUnCollect(map);
    }

    public void onMackUnCollectforNet(int position) {
        HashMap<String, String> map = new HashMap<>();
        map.put("OPT", "69");
        map.put("user_id", Config.USER_ID);
        map.put("share_id", mData.get(position).getId());
    ...
    public void onMackCollectResult(String result) {
        ...
        try {
            ...
            shiJieAdapter.notifyDataSetChanged();
    ...
    public void endNetRequse(List<ShiJieBean.ShiJieListDetail> page) {
        ...
        shiJieAdapter.notifyDataSetChanged(mData);
        pullStatus = 0;
    }


    public void onMackResult(String result) {
