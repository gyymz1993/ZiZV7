...
public class ImageMulitVAdapter extends VirtualLayoutAdapter<ImageMulitVAdapter.ImageViewHolder> implements MyItemTouchCallback.ItemTouchAdapter {
    ...
    public void onBindViewHolder(final ImageViewHolder holder, final int position) {
        final VirtualLayoutManager.LayoutParams layoutParams = new VirtualLayoutManager.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        int width = 0, height = 0;
        int imageCount = pictures.size();
        final int displayW = DisplayUtils.getDisplayWidth(context);
        final String imageUrl = pictures.get(position);
        if (imageCount == 1) {
            width = displayW;
            height = width;
            //修改为图片本身自适应宽高
           // width = getBitmapWH(imageUrl)[0];
            //height = getBitmapWH(imageUrl)[1];
